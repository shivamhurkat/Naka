...
export default function BuyerList({
  ...
  const [data, setData] = useState<Buyer[]>(initialData);
  ...
  async function fetch(opts: {
    ...
    const res = await window.fetch(`/api/buyers?${params}`);
  ...
  async function handleConfirm() {
    ...
    try {
      if (confirmAction === "delete") {
        await window.fetch(`/api/buyers/${confirmId}`, { method: "DELETE" });
        toast.success(t("deleteSuccess"));
      } else {
        await window.fetch(`/api/buyers/${confirmId}/restore`, { method: "POST" });
        ...
        <div className="flex flex-col gap-3">
          {data.map((b) => (
            <BuyerCard
              ...
              buyer={b}
              canEdit={canEdit}
              onEdit={() => router.push(`/buyers/${b.id}`)}
              onDelete={() => openDelete(b.id)}
              onRestore={() => openRestore(b.id)}
            />
          ))}
        </div>
      )}
